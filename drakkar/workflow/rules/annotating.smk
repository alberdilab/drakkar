...
rule gtdbtk:
    ...
    threads: 8
    resources:
        mem_mb=lambda wildcards, input, attempt: max(64*1024, int(input.size_mb * 1024 * 1024) * 2 ** (attempt - 1)),
        runtime=lambda wildcards, input, attempt: max(60, int(input.size_mb * 1024) * 2 ** (attempt - 1))
