...
checkpoint binette:
    ...
    threads: 8
    resources:
        mem_mb=lambda wildcards, input, attempt: min(1000*1024,max(64*1024, (row_count(input.metabat2) + row_count(input.maxbin2) + row_count(input.semibin2) * 5) * 2 ** (attempt - 1))),
