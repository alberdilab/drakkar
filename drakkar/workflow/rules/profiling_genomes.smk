...
rule index_catalogue:
    ...
    resources:
        mem_mb=lambda wildcards, input, attempt: cap_mem_mb(max(8*1024, int(input.size_mb * 5)) * 2 ** (attempt - 1)),
        runtime=lambda wildcards, input, attempt: cap_runtime(max(10, int(input.size_mb / 1024 * 2.5)) * 2 ** (attempt - 1))
    shell:
        """
        module purge
...
rule map_to_catalogue:
    ...
    shell:
        ...
        if ! samtools quickcheck -v {output}.tmp; then
            echo "ERROR: mapping of {wildcards.sample} against the genome catalogue produced a truncated BAM." >&2
            exit 1
        fi
